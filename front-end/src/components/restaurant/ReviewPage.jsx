...
const ReviewPage = ({restaurant_id}) => {
    ...
    const onClickInsert = async() => {
        ...
        await axios.post('/review/insert', {});
        setContents('');
        callAPI();
    }
    ...
    const onClickUpdate = (review_id) => {
        const data = review.map(r => r.member_user_name === review_id ? {...r, isEdit:!r.isEdit} : {...r, isEdit:false});
        setReview(data);
    }

    // 댓글 수정칸 변경 시
    const onChangeContent = (e, review_id) => {
        const data = review.map(r => r.member_user_name === review_id ? {...r, contents:e.target.value} : r);
        setReview(data);
    }
    ...
    const getReviewRating = (rating, review_id) => {
        ...
        const data = review.map(r => r.member_user_name === review_id ? {...r, rating:rating}:r);
        setReview(data);
    }
                    ...
                    {review.map(r=>
                        <div key={r.review_id}>
                            <Row style={{fontsize:'15px'}} className='mb-2'>
                                <Col xs={8} md={8} lg={8}>
                                    <span className='me-1'>{r.member_user_name}({r.member_user_uid})</span>
                                    <Stars size={18} number={r.rating} disabled={(!uid === r.member_user_uid || !r.isEdit) && true}
                                ...
                                {uid === r.member_user_uid && !r.isEdit &&
                                <Col className='text-end'>
                                    <Button variant='outline-dark' size='sm' className='pt-2 me-2' onClick={()=>onClickUpdate(r.member_user_uid)}>수정</Button>
                                    <Button variant='outline-secondary' size='sm' className='pt-2' onClick={()=>onClickDelete(r.member_user_uid)}>삭제</Button>
                                </Col>
                                }
                                {uid === r.member_user_uid && r.isEdit &&
                                <Col className='text-end'>
                            ...
                            {r.isEdit ? 
                            <div>
                                <Form.Control as="textarea" rows={5} value={r.contents} onChange={(e)=>onChangeContent(e, r.member_user_uid)}/>
                            </div>
                            :
                            <div className={r.isEllip ? 'ellipsis' : undefined} style={{cursor:'pointer', whiteSpace:'pre-wrap'}} onClick={()=>onClickContents(r.member_user_uid)}>
                                {r.contents}
                            </div>
                            }
                            <hr/>
