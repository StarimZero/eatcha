...
var db = require("../db");
var { checkAndAwardBadge } = require('./badge');
var { checkAndAwardBadge2 } = require('./badge');
var { checkAndAwardBadge3 } = require('./badge');
...
router.post('/insert', async function (req, res) {
    ...
    db.get().query(sql, [member_user_uid, contents, restaurant_id, rating], async function (err, rows) {
        if (err) {
            res.send({ result: 0 });
            console.log('리뷰등록 오류 : ', err);
        } else {
            await checkAndAwardBadge(member_user_uid)
            await checkAndAwardBadge2(member_user_uid,type)
            await checkAndAwardBadge3(member_user_uid,category)
            res.send({ result: 1 });
        }
    });
});
